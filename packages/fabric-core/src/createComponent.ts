...
export function createComponent<TProps extends object>(type: string, Component: (props: TProps) => FabricNode): ComponentBuilder<TProps> {
  return (...args) => {
    const fn: FabricComponent<TProps> = (() => intrinsic(Component(args[0] as TProps) as FabricNode)) as any
    ...
    fn.children = (...children: Array<FabricNode>) => {
      ...
      const fnChild = (() => intrinsic(Component(propsWithChildren) as FabricNode)) as FabricElement<TProps>
